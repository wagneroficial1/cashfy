...
export const Card: React.FC<CardProps> = ({ className, children, ...props }) => (
  <div className={cn("bg-slate-800 border border-slate-700 rounded-xl shadow-sm", className)} {...props}>
    {children}
  </div>
);
...
export const Input: React.FC<InputProps> = ({ className, label, id, ...props }) => (
  <div className="flex flex-col gap-1.5">
    ...
    <input
      id={id}
      className={cn("bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all", className)}
      {...props}
    />
  </div>
);
